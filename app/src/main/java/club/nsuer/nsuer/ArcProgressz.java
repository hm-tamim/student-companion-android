...
public class ArcProgressz extends View {
    ...
    public ArcProgressz(Context context) {
        this(context, null);
    }

    public ArcProgressz(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ArcProgressz(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setProgress(float progress) {
        ...
        try {
            ...
            String nn = Float.toString(progress);

            String[] splitter = nn.split("\\.");

            int decimalLength = splitter[1].length();  // After Decimal Count

            if ((progress - (int) progress) != 0)
                this.progress = Float.valueOf(new DecimalFormat("#.00").format(progress));
            else
                this.progress = Float.valueOf(new DecimalFormat("#.00").format(progress));


            if (decimalLength < 3)
                this.progress = Float.valueOf(new DecimalFormat("0.00").format(progress));
    ...
    public void setProgressBack(float progressBack) {
